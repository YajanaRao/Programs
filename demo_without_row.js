...
function getDataFromDB(hash) {
  ...
  $.ajax({
    type: "GET",
    url: "http://13.59.196.185:81/config/grafana.php",
    async: false,
    data: {
      // id: '90'
      hash:hash
...
// panel with multiple queries
function jmeterLoadInAllRegion(database,measurement,data) {
  console.log(data);
  var target = []
  for (var i = 0; i < data.length; i++){
    console.log(data[i]['region_name']);
...
function listDataSource(){
  var url='http://admin:admin@13.59.196.185:3000/api/datasources';
  var datasource;
  var response = $.ajax({
    type: "GET",
    url: url,
...
if(data != null) {
   ...
   datasourceMultiPanel(data['company'],data['project'],data['region']);
   datasourcePanelFromTemplate(data['company'],data['project']);
...
// $("body").append("<h1><p>Some appended text.</p></h1>");
return dashboard;
